...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  const urllink = `${req.protocol}://${req.get('host')}/dashboard`;
  // console.log(urllink);
  await new AllEmail.Email(newUser, urllink).sendWelcome();
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  try {
    await new AllEmail.Email(user, resetURL).sendPasswordReset();
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError(
        'Token is invalid or has expired. Request for a new one',
        400
      )
    );
  }
